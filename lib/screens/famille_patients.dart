    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text(
              'Entrez l\'email du médecin pour ce patient. Le médecin devra accepter.',
              style: GoogleFonts.roboto(),
            ),
            const SizedBox(height: 16),
            TextField(
        ...
        actions: [
          ...
          FilledButton(
            onPressed: () async {
              if (emailController.text.trim().isNotEmpty) {
                final doctorEmail = emailController.text.trim();
                ...
                await FirebaseFirestore.instance
                  ...
                  'patientFromFamille': FieldValue.arrayUnion([patientEmail]),
                });

                Navigator.pop(context);
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text('${t.requestSent} $doctorEmail')),
                );
              }
            },
            child: Text(t.sendRequest),
